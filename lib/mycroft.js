...
var defaults = {
    ...
    repl_host: 'localhost',
    repl_port: 9999,
    repl_prompt: 'mycroft@'+os.hostname()+'>'
};
...
mycroft.start = function(options) {
    ...
    config.repl_host = options.repl_host || defaults.repl_host;
    config.repl_port = options.repl_port || defaults.repl_port;
    config.repl_prompt = options.repl_prompt || defaults.repl_prompt;
    ...
    if (config.log_file) {
        var stream = fs.createWriteStream(config.log_file, {flag: 'a', mode: '0644'});
        console.error = function(msg) {
...
mycroft.startRepl = function() {
    var self = this;
    this.repl_server = net.createServer(function(socket) {
        self.repl = repl.start(config.repl_prompt, socket);
        self.repl.context.mycroft = mycroft;
        var keys = Object.keys(plugins);
        for (var i = 0; i < keys.length; i++) {
            var key = keys[i];
            self.repl.context[key] = plugins[key];
        }
        self.repl.context.nodes = nodes;
    });
    this.repl_server.listen(config.repl_port, config.repl_host);
};
